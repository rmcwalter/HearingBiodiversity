...
for k = 1:length(i)
    ...
    SXn_temp(k)     = SXn(i(k),j(k));       % std of proportion correct
    MXn_std_temp(k) = MXn_std(i(k),j(k));   % alternative std measure
    SXn_std_temp(k) = SXn_std(i(k),j(k));
    MXi_temp(k)     = MX_ind_mean(i(k),j(k)); % individual mean
    MXi_std_temp(k) = MX_ind_se(i(k),j(k));   % individual SE
end
