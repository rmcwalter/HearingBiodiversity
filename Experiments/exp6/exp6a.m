function exp6a
% EXP6A  Exemplar discrimination with geophony background (Experiment 6a).
%   Loads subject data, plots mean proportion correct across chorus sizes
%   (isolated and mixed with geophony background), and runs repeated
%   measures ANOVAs to test effects of chorus size and background type.
%
%   Data variables loaded from Exp6a.mat:
%     MX_iso  - subjects x chorus sizes (1,8,32), isolated (no background)
%     MX_mix  - subjects x chorus sizes (1,8,32), mixed with geophony
...
bar(8,udm([7]),'facecolor',c(3,:),'facealpha',1);   % background-only condition
...
%%  --- rmANOVA 1: effect of chorus size (isolated condition) ---
...
%%  --- rmANOVA 2: effect of stimulus type at largest chorus size ---

% Compare: mix@C32, iso@C32, and background-only at matched chorus size
data = array2table([MX_mix(:,3) MX_iso(:,3) MX_bg'], 'VariableNames', {'C1', 'C2', 'C3'});
% Within-subjects factor: mixture type (mixed, isolated, background-only)
