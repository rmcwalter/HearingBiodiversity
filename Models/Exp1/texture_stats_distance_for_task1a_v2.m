function texture_stats_distance_for_task1a_v2
% Compute texture-statistic distances for Experiment 1a (sound families 1-11)
% and simulate the 3-AFC discrimination task.
%
% Pipeline:
%   1. Measure auditory texture statistics for each sound file and save to X1.mat
%   2. Z-score statistics across sounds and compute pairwise distances (p1_v2.mat)
%   3. Simulate the 3-AFC task and reshape performance by N1/N2 condition
%   4. Plot model predictions against human data (loaded from v1a.mat)
%
...
for k = 1:11  % process sound families 1-11 (task 1a); change limit to length(ds) for all
...
%%
% --- Distance computation (can be run independently by loading X1.mat) ---
clear all
load X1.mat
...
%%
% --- Task simulation (can be run independently by loading p1_v2.mat) ---
clear all
load('p1_v2.mat')
...
% The 360 trials per sound family are arranged as a 6x6x10 grid (N1 x N2 x repeat).
% v{d} collects the off-diagonal at lag d (pairs with |N1-N2| = d levels).
% v{6} holds the diagonal (N1 == N2 conditions).
for n = 1:size(p,1)
    ud1 = reshape(CR(n,1:360),6,6,10);
    udm(:,:) = mean(ud1,3);

    v{1}(:,n) = mean([udm(1,2:end);udm(2:end,1)']);
    v{2}(:,n) = mean([udm(2,3:end);udm(3:end,2)']);
    v{3}(:,n) = mean([udm(3,4:end);udm(4:end,3)']);
    v{4}(:,n) = mean([udm(4,5:end);udm(5:end,4)']);
    v{5}(:,n) = mean([udm(5,6:end);udm(6:end,6)']);
    v{6}(:,n) = [udm(1,1) udm(2,2) udm(3,3) udm(4,4) udm(5,5) udm(6,6)];
end

for n = 1:length(v)
    vm{n} = mean(v{n},2); % mean over sound families
end
...
figure(1)
c = colororder;
c = c([3:6 2 2],:);     % one colour per off-diagonal lag group (lags 1-5) + diagonal
...
% x-axis positions for each off-diagonal lag group (offset -0.2 for model)
xL{1} = [1:5]-0.2;
xL{2} = [7:10]-0.2;
xL{3} = [12:14]-0.2;
xL{4} = [16:17]-0.2;
...
dof = 0;
for n = 1:length(xL)-1
    se = std(v1a.v{n},[],2)/sqrt(size(v1a.v{n},2)); % human SEM across participants
    ...
    if n < 5
        ...
        patch([xL{n} fliplr(xL{n})],[v1a.vm{n}'+se' fliplr(v1a.vm{n}'-se')],c(n,:),'FaceAlpha',0.2,'EdgeAlpha',0)
...
xL{6} = 1:6;
sd = std(v{6},[],2);
patch([xL{6} fliplr(xL{6})],[vm{6}'+sd'./2 fliplr(vm{6}'-sd'./2)],c(5,:),'FaceAlpha',0.2,'EdgeAlpha',0)
plot(xL{6},vm{6},'-.','Color',c(5,:),'linewidth',2)
pp(1) = plot(xL{6},vm{6},'o','MarkerFaceColor',dx,'MarkerEdgeColor',c(5,:),'MarkerSize',10,'linewidth',1.5);

se = std(v1a.v{6},[],2)/sqrt(size(v1a.v,2));
patch([xL{6} fliplr(xL{6})],[v1a.vm{6}'+se' fliplr(v1a.vm{6}'-se')],c(5,:),'FaceAlpha',0.2,'EdgeAlpha',0)
plot(xL{6},v1a.vm{6},'-','Color',c(5,:),'linewidth',2)
pp(2) = plot(xL{6},v1a.vm{6},'o','MarkerFaceColor',c(5,:),'MarkerEdgeColor',dx,'MarkerSize',10,'linewidth',1.5);
