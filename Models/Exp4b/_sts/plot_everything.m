...
% plot_everything  Diagnostic plot comparing target and synthetic texture
% statistics during the synthesis iteration loop.
%
% Plots subband envelope statistics (power, mean, coeff. of var.,
% skewness, kurtosis), envelope correlation matrices, and modulation
% power matrices for both the target (X) and synthetic (Y) signals.
% SNR values from st_stats are shown in each subplot title.
%
% Inputs:   X          target statistics struct
%           Y          synthetic statistics struct
%           st_stats   vector of SNR values from Measure_Statistics
%           mfb_mode   modulation filterbank spacing string
%           j          current iteration number (displayed in figure)
...
for k = 1:size(X.Mx, 2)+1
    subplot(2, 5, spk(k))
    if k == 1
        plot(fcc, X.Px);  hold on;  plot(fcc, Y.Px, 'r')
    else
        plot(fcc, X.Mx(:,k-1));  hold on;  plot(fcc, Y.Mx(:,k-1), 'r')
    end
...
% Modulation power matrices (normalised by white noise baseline)
x     = whitenoise(fs*2);
