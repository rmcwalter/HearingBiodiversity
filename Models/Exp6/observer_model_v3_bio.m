function observer_model_v3_bio
% OBSERVER_MODEL_V3_BIO  Compute McDermott/Simoncelli-style sound-texture
% statistics for the biophony-background stimulus set (Exp6, condition "b")
% and package them for use by RUN_MODEL_V2.
%
% Mirrors OBSERVER_MODEL_V3_GEO exactly, but reads from geobio_stim2bY/
% and writes to _stats_bio2/ then _statz_bio2/. See that file for the
...
for k = 1:50
    d((k-1)*63+1:k*63) = dir(['geobio_stim2bY/' num2str(k) '/*.wav']);
end
...
for kk = 1:length(d)
    ...
    save(['_stats_bio2/Z' num2str(kk) '.mat'],'Y')
...
for rx = 1
    ...
    for m = 1:length(d)
        m
        load(['_stats_bio2/Z' num2str(m) '.mat'])
        Z{3*(m-1)+1} = Y{1};
        Z{3*(m-1)+2} = Y{2};
        Z{3*(m-1)+3} = Y{3};
    end
...
for n = 1:50
    ...
    save(['_statz_bio2/Z' num2str(n) '.mat'],'ZZ')
end
